...
def load_failed_effect_results(
    *,
    ...
    """
    Carrega EffectResults com falha tcnica (success=False).
    ...
    query = (
        ...
        .where(filter=FieldFilter("status", "in", [EffectStatus.ERROR.value, EffectStatus.RETRYING.value]))
        .order_by("created_at")
        .limit(limit)
    )
...
def load_effect_result(effect_result_id: str) -> EffectResult:
    """
    Carrega um EffectResult especfico pelo seu ID de documento.
    """
    db = get_firestore_client()
    ...
    if not doc.exists:
        raise FileNotFoundError(f"EffectResult com ID {effect_result_id} no encontrado.")
