...
class EnrichMetadataService:
    ...
    Servio cognitivo EXTRACT_COMPUTABLE_METADATA (v2).
    ...
    - Validao dura via Pydantic v2

    - Integrao preservada com dev_enrich
    ...
    def __init__(
        ...
        raise NotImplementedError("EnrichMetadataService is not being used in the current architecture and is pending refactor or removal.")
    ...
    def run(self, relato_id: str, relato_text: str, has_images: bool) -> None:
        ...
        try:
            ...
            logger.debug(f"Prompt construdo para relato {relato_id}: {prompt[:100]}...")
            ...
            # TODO to be removed after monitoring: log detalhado do sucesso para anlise
            ...
            # Falha explcita (nada silencioso)
            ...
            # TODO to be removed after monitoring: log detalhado da falha para anlise (sem expor dados sensveis)
    ...
    # Classificao mnima de falhas (pode evoluir depois)
    ...
    def _is_retryable(self, exc: Exception) -> bool:
        ...
        # Violao de schema = falha cognitiva definitiva
