...
Servios de autenticao centrados em Firebase Auth + Firestore.



Nota Tcnica:

Este servio utiliza o Google Cloud Firestore SDK sncrono. Em um ambiente FastAPI de alta concorrncia,

isso pode causar bloqueio da event loop. Para o MVP, o impacto  mitigado pelo baixo volume de IO

de autenticao, mas para escala massiva, recomenda-se a migrao para 'google-cloud-firestore' async

ou execuo destas chamadas em threads separadas via 'run_in_executor'.
...
def verify_firebase_token(provider_token: str) -> dict[str, Any]:
    ...
    Valida o Firebase ID token com verificaes estritas e suporte ao Emulator.
    ...
    # 2. Validao Oficial via Firebase Admin SDK

    try:

        # check_revoked=True garante que o token no foi revogado
        ...
        logger.error(f"Falha na verificação do token Firebase: {exc}")
        raise HTTPException(
    ...
    # 4. Validao Adicional: aud e iss (se no for emulator)
...
async def get_or_create_internal_user(firebase_data: dict[str, Any]) -> User:
    ...
    Resolve ou provisiona automaticamente o perfil do usurio em Firestore.

    """

    db = get_firestore_client()  # Acesso explcito ao cliente
    ...
    # Dados bsicos sempre sincronizados com o Firebase Auth
    ...
    if user_doc.exists:
        ...
        # Garante integridade de campos obrigatrios se estiverem corrompidos
