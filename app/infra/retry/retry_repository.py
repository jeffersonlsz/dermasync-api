...
def load_failed_effect_results(
    *,
    ...
    """
    Carrega EffectResults falhos e elegveis para retry.
    NÃO decide retry.
    """

    db = get_firestore_client()
    if not db:
        raise RuntimeError("Firestore client indisponvel")

    query = db.collection("effect_results").where(
        filter=FieldFilter("status", "in", [EffectStatus.ERROR.value, EffectStatus.RETRYING.value])
    )

    if since_minutes is not None:
        since_dt = datetime.utcnow() - timedelta(minutes=since_minutes)
        query = query.where(filter=FieldFilter("created_at", ">=", since_dt))
