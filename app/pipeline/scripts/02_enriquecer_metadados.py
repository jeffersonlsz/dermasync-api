...
def gerar_prompt(texto):
    return (
        "Extraia idade, gÍnero, principais sintomas, regiıes do corpo afetadas, medicamentos e duraÁ„o de uso, produtos naturais e terapias realizadas. "
        "Ignore nomes prÛprios. Responda apenas em JSON, se n„o encontrou o dado, preencha com 'ausente':\n"
        '{"idade": ..., "genero": ..., "sintomas": [...], "regioes_afetadas": [...], '
        '"produtos_naturais": [...], "terapias_realizadas": [...], '
        '"medicamentos": [{"nome": "...", "frequencia": "...", "duracao": "..."}]}, '
        '"resumo_descritivo: "aqui È um resumo pequeno sobre o que o texto diz, para servir como titulo e subtitulo"   \n\n'
        f"TEXTO:\n{texto}"
    )
...
def processar_relatos(relatos, llm, src="local-youtube"):
    ...
    for item in tqdm(relatos):
        ...
        try:
            ...
            print(f"‚ùå Erro no relato {item['nome_arquivo']}: {e}")
    return saida
...
if __name__ == "__main__":
    ...
    if not relatos:
        print("‚ùó Nenhum relato encontrado no arquivo JSONL.")
        exit(1)
