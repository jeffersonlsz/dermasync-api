...
def normalizar_tag(tag: str) -> str:
    # Remove acentos, lowercase, troca espaos por _
...
def expandir_tags(tags_raw) -> dict:
    """
    Recebe uma lista de tags (ou string separada por vrgula) e retorna um dicionrio com chaves booleanas.
...
def montar_prompt(pergunta, resultados):
    base = f"""Abaixo, seguem uns trechos de relatos de pessoas dos seus tratamentos para dermatite atpica:\n"""

    for i in range(len(resultados["documents"][0])):
        texto = resultados["documents"][0][i]
        base += f"\nTrecho {i+1}: {texto}"
    base += f"""\n\nCom o que voc leu acima, reuna as informaes em formato de texto direto, com pargrafos, com linguagem amigavel, sobre o que viu acima, mas sem perder nenhuma informao. 
            E tambm responda esse questionamento ao final: {pergunta} \n\n"""
    return base
