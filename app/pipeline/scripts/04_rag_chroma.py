...
def normalizar_tag(tag: str) -> str:
    # Remove acentos, lowercase, troca espaÁos por _
...
def expandir_tags(tags_raw) -> dict:
    """
    Recebe uma lista de tags (ou string separada por vÌrgula) e retorna um dicion·rio com chaves booleanas.
...
def montar_prompt(pergunta, resultados):
    base = f"""Abaixo, seguem uns trechos de relatos de pessoas dos seus tratamentos para dermatite atÛpica:\n"""

    for i in range(len(resultados["documents"][0])):
        texto = resultados["documents"][0][i]
        base += f"\nTrecho {i+1}: {texto}"
    base += f"""\n\nCom o que vocÍ leu acima, reuna as informaÁıes em formato de texto direto, com par·grafos, com linguagem amigavel, sobre o que viu acima, mas sem perder nenhuma informaÁ„o. 
            E tambÈm responda esse questionamento ao final: {pergunta} \n\n"""
    return base
