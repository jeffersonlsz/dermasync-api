...
"""
Routes para relatos (depoimentos de tratamento de dermatite atÛpica).
"""
import logging
import uuid
import json
...
@router.post(
    ...
    summary="Enviar relato (rota canÙnica baseada em domÌnio)",
    tags=["Relatos"],
)
    ...
    *,
    ...
    if not draft.consentimento:
        raise HTTPException(
            ...
            detail="Consentimento È obrigatÛrio",
        )
    ...
    if not decision.allowed:
        raise HTTPException(
            ...
            detail=decision.reason or "CriaÁ„o do relato n„o permitida",
        )
...
async def moderate_relato_route(
    ...
    """
    Executa uma aÁ„o de moderaÁ„o em um relato, delegando a lÛgica para o domÌnio.

    - **action**: A aÁ„o a ser executada (`approve`, `reject`, `archive`).
    
    Apenas usu·rios com as roles 'admin' ou 'colaborador' podem executar esta aÁ„o.
    A lÛgica de negÛcio real (ex: um relato sÛ pode ser aprovado se estiver no estado 'processed')
    È garantida pela camada de domÌnio.
    """
    # A verificaÁ„o de role na rota È uma primeira barreira (defense-in-depth),
    # mas a verdadeira autorizaÁ„o acontece no domÌnio.
...
@router.get(
    "/moderation/pending",
    summary="Lista relatos pendentes de moderaÁ„o",
    tags=["Relatos"]
)
...
@router.get(
    "/{relato_id}/imagens",
    summary="Imagens associadas ao relato (contrato canÙnico)",
    tags=["Relatos"]
)
async def get_imagens_relato(
    ...
    """
    Endpoint canÙnico para consumo de imagens pelo front-end.
    """
