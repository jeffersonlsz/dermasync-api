...
class ImagensSchema(BaseModel):
    ...
    durante: List[str] = Field(default=[], description="Lista de URLs de imagens durante o tratamento")
    depois: Union[str, None] = Field(default=None, description="URL da imagem depois do tratamento (pode ser null)")
...
class RelatoCompletoInput(BaseModel):
    ...
    sintomas: List[str] = []
    imagens: ImagensSchema  # Obrigatório (já que 'antes' é obrigatório dentro do schema)
