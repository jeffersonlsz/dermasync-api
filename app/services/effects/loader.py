...
def load_failed_effect_results(
    *,
    ...
    """
    Carrega EffectResults com falha tÃ©cnica (success=False).

    NÃƒO decide retry.
    NÃƒO executa efeitos.
    NÃƒO altera estado.
    """
...
def load_effect_result(effect_result_id: str) -> EffectResult:
    """
    Carrega um EffectResult especÃ­fico pelo seu ID de documento.
    """
    db = get_firestore_client()
    ...
    if not doc.exists:
        raise FileNotFoundError(f"EffectResult com ID {effect_result_id} nÃ£o encontrado.")
