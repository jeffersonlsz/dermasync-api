...
def normalize_firestore_value(value: Any) -> Any:
    """
    Normaliza recursivamente valores para serem compatÌveis com o Firestore.
    ...
    # MantÈm outros tipos primitivos (str, int, bool, None, datetime)
    return value
...
def persist_effect_result_firestore(
    ...
    """
    Persiste o resultado tÈcnico de execuÁ„o de um efeito.

    N√ÉO lanÁa exceÁ„o.
    N√ÉO interfere no fluxo.
    """
    ...
    try:
        ...
        # ‚ö†Ô∏è Nunca quebrar o fluxo principal
        logger.error(
