...
# ============================================================
# üîπ SeleÁ„o de thumbnails (ANTES / DEPOIS)
# ============================================================

def _pick_thumbnails(imagens: List[dict]) -> Dict[str, str | None]:
    """
    Retorna thumbnails separadas por papel clÌnico.
    Prioridade:
      - ANTES
      - DEPOIS
    """
...
# ============================================================
# üß† ServiÁos Cognitivos (legacy-safe)
# ============================================================
...
async def listar_galeria_publica_v3(
    *,
    ...
    # --------------------------------------------------------
    # 3Ô∏è‚É£ Montar resposta final (D2 ATIVO)
    # --------------------------------------------------------
    ...
    for relato_id, relato in relatos:
        ...
        # =====================================================
        # üß† PIPELINE COGNITIVO ‚Äî D2 (exposiÁ„o progressiva)
        # =====================================================

        # PolÌtica mÌnima (n„o muda comportamento)
        ...
        eligibility_decision = eligibility_service.decide(
            user=None,  # galeria p˙blica
            relato_policy=visibility_policy,
        )

        # üîπ Similaridade neutra (placeholder controlado)
...
async def listar_galeria_contextual(
    *,
    ...
    # ============================================================
    # 1Ô∏è‚É£ Resolver perfil cognitivo do usu·rio
    # ============================================================

    # üîπ STUB CONTROLADO (v1)
    ...
    # ============================================================
    # 2Ô∏è‚É£ Buscar relatos candidatos (p˙blicos)
    # ============================================================
    ...
    # ============================================================
    # 3Ô∏è‚É£ AvaliaÁ„o cognitiva por relato
    # ============================================================
    ...
    for relato_id, relato in relatos:
        ...
        # --------------------------------------------------------
        # üîπ Similaridade v1 (simples e honesta)
        # --------------------------------------------------------

        # STUB: partial scores simples
        partial_scores = {
            # exemplos: substituir depois por lÛgica real
    ...
    # ============================================================
    # 4Ô∏è‚É£ Ordenar por similaridade
    # ============================================================
...
async def resolve_relato_base_for_user(
    *,
    ...
    """
    Resolve o relato-base cognitivo do usu·rio.

    PolÌtica v1 (congelada):
    - relato mais recente
    - status aprovado
    - com tags_extraidas
    - usado como ‚ncora de similaridade

    Retorna apenas os campos necess·rios para o pipeline cognitivo.
    """
    ...
    if not tags:
        return None
    # TODO revisar se isso est· correto depois. 
