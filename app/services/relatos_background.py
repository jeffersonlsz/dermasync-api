...
Esta camada NÃO deve conter lgica de negcio.
...
def _update_status_domain(relato_id: str, action: str, error_message: Optional[str] = None):
    ...
    Funo interna para mudar o status de um relato via camada de domnio.
    ...
    if not doc.exists:
        ...
        logger.error(f"[BG_DOMAIN] Relato {relato_id} no encontrado para atualizao de status.")
    ...
    if not current_status_str:
        ...
        logger.error(f"[BG_DOMAIN] Relato {relato_id} no possui status.")
    ...
    if not command:
        ...
        logger.error(f"[BG_DOMAIN] Ao de background desconhecida: {action}")
    ...
    if not decision.allowed:
        ...
        logger.error(f"[BG_DOMAIN] Transio de estado no permitida para o relato {relato_id}: {decision.reason}")
...
def _save_files_and_enqueue(
    ...
    *,
    ...
    Processa o upload de imagens em background e atualiza o status do relato via domnio.

    """

    from app.services.imagens_service import salvar_imagem_bytes_to_storage

    from app.application.relatos.submit_relato_use_case import enqueue_relato_processing
    ...
    try:
        ...
        # Notifica o domnio que o upload foi concludo

        _update_status_domain(relato_id=relato_id, action="uploaded")



        # Enfileira o prximo passo do processamento

        # Idealmente, isso tambm seria um comando de domnio que retorna um EnqueueEffect
