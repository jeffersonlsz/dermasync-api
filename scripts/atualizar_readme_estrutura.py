...
print(f"ð± Iniciando atualização do README na pasta: {ROOT}")
...
def carregar_arquivo_mermaid():
    ...
    return "â ï¸ Diagrama em texto (Mermaid) não disponível.\n"
...
def atualizar_readme():
    titulo = "# ð± Projeto DermaSync\n"
    imagem_arquitetura = "![Arquitetura DermaSync](docs/arquitetura-dermasync.png)\n"
    print("ð Atualizando README.md com a nova estrutura...")
    ...
    print(f"ð³ Estrutura de pastas gerada com sucesso. {arvore}")
    resumo = gerar_resumo_tecnico()
    print("ð§ Resumo técnico gerado com sucesso.")
    diagrama = gerar_diagrama_mermaid()
    print("ð Diagrama Mermaid gerado com sucesso.")
...
## ð Estrutura de Pastas
```text
{arvore}
```

## ð Detalhes do Projet
# ð± Projeto DermaSync â Estrutura Atualizada
{imagem_arquitetura}
## ð Atualização do README
ð Ãltima atualização automática: {datetime.datetime.now().strftime("%d/%m/%Y %H:%M:%S")}
"""
...
def carregar_imagem_diagrama():
    ...
    return "â ï¸ Diagrama visual ainda não disponível.\n"
...
def gerar_diagrama_mermaid():
    ...
    if mermaid_diagram:
        ...
        print("â ï¸ Diagrama Mermaid não encontrado ou vazio.")
        return ""
