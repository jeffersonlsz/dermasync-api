...
@pytest.mark.asyncio
async def test_external_login_alias_uses_same_contract(client: AsyncClient, mocker):
    user = User(
        id="uid_999",
    ...
    response = await client.post(
        "/auth/external-login",
        json={"provider_token": "firebase-token"},
    )

    assert response.status_code == status.HTTP_200_OK
    assert response.json()["user"]["user_id"] == "uid_999"
