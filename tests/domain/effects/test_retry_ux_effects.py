...
# =========================================================
# TEST 1 â Ignora quando não há effects falhos
# =========================================================

def test_retry_emits_retry_ux_effect(monkeypatch):
    monkeypatch.setattr(
        "app.infra.retry.relato_retry.fetch_failed_effects",
        lambda relato_id: [],
    )
...
def test_retry_executes_failed_effects(monkeypatch):
    ...
    monkeypatch.setattr(
        "app.application.effects.retry_effect_executor.resolve",
...
@pytest.mark.asyncio
async def test_retry_endpoint_returns_ux_effect(
    client,
    mock_current_user_usuario_logado,
):
    response = await client.post("/relatos/relato-123/retry")
