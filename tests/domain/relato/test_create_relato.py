...
def test_post_relatos_success_runs_domain_and_executes_effects():
    ...
    # ð override correto do Depends
    app.dependency_overrides[get_current_user] = lambda: mock_user

    client = TestClient(app)

    # â payload MINIMAMENTE válido segundo RelatoDraftInput
    payload = {
    ...
    try:
        with patch(
            "app.application.effects.dispatcher.EffectDispatcher.dispatch"
        ) as mock_dispatch:
...
def test_post_relatos_admin_is_allowed_by_domain():
    ...
    try:
        with patch("app.application.effects.dispatcher.EffectDispatcher.dispatch") as mock_dispatch:
            mock_dispatch.return_value = None
