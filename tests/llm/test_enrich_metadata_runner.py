...
def test_run_enrich_metadata_llm_empty_relato():
    with pytest.raises(ValueError, match="Relato vazio ou invÃ¡lido."):
...
def test_run_enrich_metadata_llm_invalid_json(mock_ollama_client):
    ...
    # Retorna uma string que nÃ£o Ã© um JSON vÃ¡lido
...
def test_run_enrich_metadata_llm_not_dict_json(mock_ollama_client):
    ...
    with pytest.raises(ValueError, match="Resposta do LLM nÃ£o retornou JSON vÃ¡lido."):
        run_enrich_metadata_llm("Valid relato text")
...
def test_parse_llm_response_thinking_output():
    ...
    thinking_response = (
        ...
        '{"idade": null,"genero": null,"sintomas": ["dermatite at�pica","coceira"],'
    ...
    expected_dict = {
        "idade": None,
        "genero": None,
        "sintomas": ["dermatite at�pica", "coceira"],
        "tratamentos_mencionados": ["hidratar a pele"]
    }
