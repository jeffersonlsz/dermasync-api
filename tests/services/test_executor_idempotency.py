...
def test_executor_skips_effect_if_already_succeeded(monkeypatch):
    """
    Prova que o executor N√ÉO executa novamente
    ...
    # -----------------------------
    # Simula idempotÍncia ativa
    # -----------------------------
    def fake_effect_already_succeeded(*, relato_id, effect_type, effect_ref):
        # Simula que o efeito J√Å FOI executado com sucesso
        return True
    ...
    assert upload_call_count == 0, (
        "Upload N√ÉO deveria ser chamado se o efeito "
